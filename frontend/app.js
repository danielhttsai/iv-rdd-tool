...
const DNODES = {
  ...
  rNCC: { rec: { kind: "external", badge: "↗",
    ...
    why: { zh: "巢式對照只對「個案＋抽樣對照」量暴露，所以特別適合<b>需要費工測量「暴露量／劑量」</b>的研究——量得起就能畫出<b>劑量–反應（dose-response）</b>關係，而結果≈完整世代分析。（純粹「省成本」其實案例交叉也做得到，所以這裡的關鍵是<b>劑量–反應</b>。）",
           en: "Nested case-control measures exposure only for cases + sampled controls, so it shines when the exposure <b>level/dose is expensive to measure</b> — you can afford the detailed measurement and map a <b>dose-response</b> relationship, with results ≈ the full-cohort analysis. (Plain cost-saving alone is also true of case-crossover, so the real edge here is <b>dose-response</b>.)" },
